...
const AuctionListing: React.FC<AuctionListingProps> = ({
  ...
  const viewListing = () => {
    navigate(`/listing/${_id}`, { state: { listing: { _id, title, charityId, description, status, categoryId, auctionStartDate, auctionEndDate, currentPrice, image } } });
  };

  const renderImage = () => {
    try {
      if (image instanceof Blob) {
        return <img src={URL.createObjectURL(image)} alt={title} className={styles.listingImage} />;
    ...
    <div className={`${styles.listing} ${view === 'grid' ? styles.gridItem : styles.listItem}`}>
      ...
      <div className={styles.listingContent}>
        ...
        <div className={styles.auctionDetails}>
          ...
          <button className={styles.bidButton} onClick={viewListing}>Place Bid</button>
        </div>
      </div>
    </div>
  );
